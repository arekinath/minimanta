...
FstorObject.prototype.getAtomicWriteStream = function (writecb, cb) {
	...
	function setupStream(_, ccb) {
		...
		function finish() {
			if (!ptserr) {
				mod_assert.string(md5data);
				tmp.openWrite(function (err) {
					tmp.setMetadata('content-md5', md5data);
					self.setMetadata('content-md5', md5data);
					ccb(err);
				});
			}
		}
...
FstorObject.prototype.mkdir = function mkdir(cb) {
	...
	mod_assert.ok(parent.fo_locked);
...
FstorObject.prototype.writeDirToStream = function (str, limit, marker, cb) {
	var self = this;
	var done = 0;
	mod_assert.ok(this.isDirectory());
	this.getChildren(function (err, kids) {
		...
		function writeKidJson(kid, ccb) {
			if (marker && kid.getBasename() < marker) {
				ccb();
				return;
			}
			if (limit && isFinite(limit) && done > limit) {
				ccb();
				return;
			}
